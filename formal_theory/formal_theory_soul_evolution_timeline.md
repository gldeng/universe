...
1. **连续性**：$`\forall t_i, t_j, |t_i - t_j| < \epsilon \Rightarrow |\mathcal{S}^{t_i} \oplus \mathcal{S}^{t_j}| < \delta`$
...
其中 $`E(\mathcal{S}^t)`$ 是灵魂经验值，$`C_{max}`$ 是理论最大复杂度。
...
其中 $`E(\mathcal{S}^t)`$ 是路径能量，$`\lambda`$ 是权重系数。
