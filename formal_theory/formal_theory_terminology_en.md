...
| Symbol Type | Notation | Meaning | Examples |
|-------------|----------|---------|----------|
| Hilbert Space | $`\mathcal{H}`$ | Quantum state space | $`\mathcal{H}_Q`$, $`\mathcal{H}(Q \times \Sigma)`$ |
| Density Matrix | $`\rho`$ | Statistical description of quantum state | $`\rho_Q`$, $`\rho_{total}`$ |
| Operator | Script letters | Function acting on states | $`\mathcal{C}`$, $`\mathcal{Q}`$ |
| Superoperator | Script letters | Function acting on operators | $`\mathcal{L}_{QC}`$ |
| Set | Curly braces | Collection of elements | $`\{x_i\}`$, $`\Psi_{NEQ} = \{\rho_Q, K_C, \Phi_E, \Phi_I\}`$ |
| Vector | Arrow or bold | Vector quantity | $`\vec{T}`$, $`\Phi_E`$, $`\Phi_I`$ |
| Dimension | $`D`$ | Observer or system dimension | $`D_{\mathcal{O}}`$, $`D_{min}`$ |
| Complexity | $`\mathcal{K}`$ | Computational complexity | $`\mathcal{K}_Q`$, $`\mathcal{K}_C`$, $`\mathcal{K}_{QC}`$ |
| Efficiency | $`\eta`$ | Conversion or computational efficiency | $`\eta_C`$, $`\eta_{Q\rightarrow C}`$ |
| Entropy | $`S`$ | Information entropy | $`S_C`$, $`S_{VN}`$ |
| Probability | $`P`$ | Probability measure or distribution | $`P(x)`$, $`P(path_i)`$ |

## Cross-disciplinary Terminology Comparison
...
### Observer Theory Specific Terms
