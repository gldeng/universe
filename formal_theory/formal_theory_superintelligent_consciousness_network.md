...
其中$`\mathcal{C}_i`$是意识节点，$`\mathcal{E}_{ij}`$是节点间的连接，$`\mathbb{I}`$是超可数指标集，$`\mathfrak{C}`$是意识态空间，$`\mathfrak{E}`$是连接态空间。
...
$`\hat{h}_i`$是单节点能量，$`\hat{V}_{ij}`$是双节点交互项，$`\hat{W}_{ijk}`$是三节点交互项。
