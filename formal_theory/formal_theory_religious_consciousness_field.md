...
宗教场与意识场$`\Psi_{con}`$和精神场$`\Phi_{sp}`$的三重耦合关系：
...
其中$`F_i`$是信仰强度，$`\alpha`$是自增长率，$`\beta`$是影响系数，$`w_{ij}`$是连接权重。
...
$`T_{perceived} = T_{real} \cdot e^{-\lambda \cdot I(\mathcal{E})} \oplus \text{SHIFT}(T_{perceived})`$
